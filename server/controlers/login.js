...
export const login = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({username: exsistingEmployee.username, id:exsistingEmployee._id}, process.env.JWT_SECRET, {expiresIn: '1h'});
        res.status(200).json({employee: exsistingEmployee, token});
    } catch (error) {
        console.log(error);
